...
class GAhostBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1):  # ch_in, ch_out, kernel, stride
        super().__init__()
        c_ = c2 // 2
        self.conv = nn.Sequential(GAConv(c1, c_, 1, 1),  # pw
                                  GAConv(c_, c2, 1, 1, act=False))  # pw-linear
        
        # self.shortcut = nn.Sequential(DWConv(c1, c1, k, s, act=False),
        #                              Conv(c1, c2, 1, 1, act=False)) if s == 2 else nn.Identity()
        self.shortcut = nn.Sequential(DWConv(c1, c1, k, s, act=False),
                                     Conv(c1, c2, 1, 1, act=False)) if c1 != c2 else nn.Identity()

    def forward(self, x):

        return self.conv(x) + self.shortcut(x)
    
class GAhostELANStack1_ema(nn.Module):
    def __init__(self, c1, c2, k=3, s=1):
        super().__init__()
...
class GhostELANStack1_ema(nn.Module):
    def __init__(self, c1, c2, k=3, s=1):
        super().__init__()
...
class EMA(nn.Module):
    def __init__(self, channels, factor=8):
        ...
        self.conv1x1 = Conv(channels // self.groups, channels // self.groups, k=1, s=1, p=0, g=1, act=False)
        self.conv3x3 = Conv(channels // self.groups, channels // self.groups, k=3, s=1, p=1, g=1, act=False)
...
class GhostConv_CSA(nn.Module):
    ...
    def forward(self, x):
        ...
        sptial_att =  torch.mean(x1, 1, keepdim=True)  # b, 1, h, w
        channel_att = torch.mean(x2, (2, 3), keepdim=True)  # b, c_, 1, 1
              
        att_x1 = torch.sigmoid(sptial_att)
        att_x2 = torch.sigmoid(channel_att)
